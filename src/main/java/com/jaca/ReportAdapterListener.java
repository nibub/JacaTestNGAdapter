...
public class ReportAdapterListener
    implements IResultListener, ISuiteListener, IInvokedMethodListener {
  ...
  static {
    ...
    try {
      ...
      if (baseUrl != null && !baseUrl.isEmpty()) {
        ...
        LOGGER.info(
            "Jaca is not configured, Set reporting dashboard base url command line parameter -- 'JURL' ");
      }
  ...
  private Map<String, HashMap<String, Integer>> groupsInfoLists =
  ...
  private void updateSuiteResults(ISuite suite) {
    ...
    try {
      ...
      for (String groupName : groupsInfoLists.keySet()) {
        ...
        int total =
            resultsStatuses.get("Pass") + resultsStatuses.get("Fail") + resultsStatuses.get("Skip");
      ...
      if (response.getStatusLine().getStatusCode() != 200) {
        LOGGER.info(
            "Suite info update failed with status code :"
    ...
    if (iTestResult.getMethod().isTest()) {
      try {
        ...
        if (iTestResult
            ...
            .getMethod()
            .isAnnotationPresent(TestAnnotations.class)) {
          testID =
              iTestResult
                  .getMethod()
                  .getConstructorOrMethod()
                  .getMethod()
                  .getAnnotation(TestAnnotations.class)
                  .testID();
          priority =
              iTestResult
                  .getMethod()
        ...
        if (iTestResult.getThrowable() != null) {
          ...
          if (basePackName != null) {
            //                    Stream<StackTraceElement > str  =
            // Arrays.stream(iTestResult.getThrowable().getStackTrace()).filter(item->item.getClassName().startsWith(""));
            Object[] str =
                Arrays.stream(iTestResult.getThrowable().getStackTrace())
                    .filter(item -> item.getClassName().startsWith(basePackName))
                    .toArray();
            if (str.length > 0) {
        ...
        if (currentClass instanceof JacaBase) {
          ...
          if (driver instanceof RemoteWebDriver) {
            ...
            try {
              ...
              LOGGER.info(
                  "Unable to capture the Driver url, Check the execution conditions,Browser might have closed/not launched");
            }
            platformInfoCaptured = true;
          } else {
            LOGGER.info(
                "Unable to find Remote Webdriver instance, Verify the driver implementation");
          }
        } else {
          LOGGER.info(
              "Unable to capture driver info, driver object info should be available (implement JacaBase Interface)");
        }
        ...
        if (iTestResult.getStatus() == ITestResult.FAILURE) {
          if (currentClass instanceof JacaBase) {
            ...
            LOGGER.info(
                "Unable to capture failure screen shot, driver object info should be available (implement JacaBase Interface)");
          }
        }
